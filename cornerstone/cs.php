...
class cs
{
    ...
    public function __construct($cli = false)
    {
        ...
        new ClassLoader(null, APP_PATH . DS . 'models');
        new ClassLoader(null, APP_PATH . DS . 'helpers');
        new ClassLoader(null, CORNERSTONE_PATH . DS . 'helpers');
        new ClassLoader(null, APP_PATH . DS . 'libraries');
        new ClassLoader(null, CORNERSTONE_PATH . DS . 'libraries');
    ...
    public function init($cli = false)
    {
        ...
        if (Config::get('useTemplate') !== false) {
    ...
    public function dispatch()
    {
        ...
        if ($route) {
            //handle custom route
            if ($route->isRedirect()) {
                RequestHelper::redirect($route->redirectLocation, $route->status);
            } elseif ($route->isCustomRouter()) {
                if ($route->executeCustomRouter()) {
                    return true;
                }
            } else {
                    ...
                    if (!is_dir(CONTROLLER_PATH . $subdomainFolder)) {
                        if (is_dir(CONTROLLER_PATH . '_*_/')) {
                            ...
                            if (empty($path)) {
                                ...
                                $controller->get();
                                die();
                            } elseif (is_dir(CONTROLLER_PATH . '_*_/' . $path) && is_readable(CONTROLLER_PATH . '_*_/' . $path . '/controller.php')) {
                                include(CONTROLLER_PATH . '_*_/' . $path . '/controller.php');
